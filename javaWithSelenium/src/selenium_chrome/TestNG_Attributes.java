...
WebDriver driver;
	...
	@Test(enabled = false,timeOut = 1000,alwaysRun = false,description = "this is sample tc")
	//if we write enabled = false, then that test case is not going to be executed
	public void again() {
		System.out.println("@Test: again");
	}
	@Test(priority = 0,invocationCount = 5,invocationTimeOut = 10000,dependsOnMethods = {"again"})
	//the test method should complete within 10sec otherwise we will get error that means test caSE IS FAILING
	//whenever u r working with invocationCount then only this invocationTimeOut come into picture
	//as rule priority this has to be executed but this depends on method so again method is executed
